...
class <%= model_controller_class_name %>ControllerTest < ActionController::TestCase
  ...
  def test_should_activate_user
    assert_nil <%= class_name %>.authenticate(<%= aaron_email_or_login %>, 'test')
    get :activate, :activation_code => <%= table_name %>(:aaron).activation_code
    assert_redirected_to '/<%= controller_routing_path %>/new'
    assert_not_nil flash[:notice]
    assert_equal <%= table_name %>(:aaron), <%= class_name %>.authenticate(<%= aaron_email_or_login %>, 'monkey')
  end
  ...
  protected
    def create_<%= file_name %>(options = {})
      post :create, :<%= file_name %> => { <% if !email_as_login? %>:login => 'quire', <% end %>:email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options)
    end
end
