...
<% end %>
  ...
  def test_should_reset_password
    ...
    assert_equal <%= table_name %>(:quentin), <%= class_name %>.authenticate(<%= quentin_email_or_login %>, 'new password')
  end

  def test_should_not_rehash_password
    <%= table_name %>(:quentin).update_attributes(:<%= email_or_login %> => <%= quentin_changed_email_or_login %>)
    assert_equal <%= table_name %>(:quentin), <%= class_name %>.authenticate(<%= quentin_changed_email_or_login %>, 'test')
  end

  def test_should_authenticate_<%= file_name %>
    assert_equal <%= table_name %>(:quentin), <%= class_name %>.authenticate(<%= quentin_email_or_login %>, 'test')
  end
...
<% if options[:stateful] %>
  ...
  def test_suspended_<%= file_name %>_should_not_authenticate
    <%= table_name %>(:quentin).suspend!
    assert_not_equal <%= table_name %>(:quentin), <%= class_name %>.authenticate(<%= quentin_email_or_login %>, 'test')
  end
...
protected
  def create_<%= file_name %>(options = {})
    record = <%= class_name %>.create({ <% if !email_as_login? %>:login => 'quire', <% end %>:email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
